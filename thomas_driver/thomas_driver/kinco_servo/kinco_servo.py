...
class KincoServo:
    ...
    def read_parameter_old(self, type, index, subindex):
        ...
        try:
            ...
            if cmd == 0x4F:
                ...
                print(f"Read failed. Error code: 0x{error_code:08X}")
                return None
            else:
                print(f"Unexpected response command: {cmd}")
                return None
        except Exception as e:
            # print(e)
            return None
            
    def read_parameter(self, type, index, subindex):

        if type == DataType.INT8:
            command = struct.pack('<BHBB3x', 0x40, index, subindex, 0)

        elif type == DataType.INT16.value: 
            command = struct.pack('<BHBH2x', 0x40, index, subindex, 0)

        elif type == DataType.UNSIGNED_INT16: 
            command = struct.pack('<BHBH2x', 0x40, index, subindex, 0)

        elif type == DataType.INT32:
            command = struct.pack('<BHBi', 0x40, index, subindex, 0)

        elif type == DataType.UNSIGNED_INT32:
            command = struct.pack('<BHBi', 0x40, index, subindex, 0)
        else:
            return      
        try:
    ...
    def write_parameter(self, type, index, subindex, value) -> bool:
        try:
            ...
            if isinstance(value, int):
                ...
                if type == DataType.INT8:
                    ...
                    # command = struct.pack('<BHBH', 0x2B, index, subindex, value)
                    ...
                    command = struct.pack('<BHBi', 0x23, index, subindex, value)
